...
主要改进：
1. 使用优化版模型（SE注意力 + 覆盖注意力 + 更深网络）
2. 更强的数据增强
3. 改进的学习率调度（Warmup + Cosine）
4. 混合损失函数（CE + Focal + Label Smoothing）
5. 梯度累积和混合精度训练
6. 更好的早停策略
7. 支持Ctrl+C安全中断并保存检查点

目标：字符准确率>=80%，公式准确率>=80%

用法：
python -m train.train_e2e_v3 --data_root "D:/CROHME/CROHME" --epochs 200 --batch_size 16 --device cuda --amp

中断训练：按Ctrl+C，会自动保存当前状态
恢复训练：添加 --resume 参数
"""
...
def signal_handler(signum, frame):
    ...
    if _interrupted:
        ...
        sys.exit(1)
    _interrupted = True
    print("\n\n⚠️  检测到中断信号 (Ctrl+C)，正在安全保存检查点...")
    print("   再次按Ctrl+C强制退出（不推荐）\n")
...
def save_checkpoint(path: Path, state: Dict) -> None:
    """保存检查点"""
    global _current_state, _checkpoint_path
...
def save_interrupt_checkpoint():
    """中断时保存检查点"""
    global _current_state, _checkpoint_path
    if _current_state is not None and _checkpoint_path is not None:
        # 保存到last.ckpt
        last_path = _checkpoint_path.parent / "last.ckpt"
        torch.save(_current_state, str(last_path))
        print(f"✓ 检查点已保存到: {last_path}")
        print(f"  当前epoch: {_current_state.get('epoch', '?')}")
        print(f"  最佳seq_acc: {_current_state.get('best_val_seq_acc', 0):.4f}")
        print(f"  最佳tok_acc: {_current_state.get('best_val_tok_acc', 0):.4f}")
        print("\n下次使用 --resume 参数继续训练")
    ...
    *,
    ...
    if resume_requested and resume_ckpt_path.exists():
        raise SystemExit(
            ...
            f"Missing tokens: {preview}. Start a fresh run without --resume or use a new checkpoint directory."
        )
...
def print_gt_vs_pred(
    ...
    """打印前n条 GT vs PRED 方便人工观察"""
    model.eval()
    printed = 0
    for images, _widths, targets in loader:
        if printed >= n_samples:
            break
        images = images.to(device, non_blocking=True)
        targets = targets.to(device, non_blocking=True)
        preds = model.greedy_decode(images, bos_id=bos_id, eos_id=eos_id, max_len=max_len)
        for i in range(min(images.size(0), n_samples - printed)):
            gt_ids = targets[i].tolist()
            pr_ids = preds[i].tolist()
            # 截断到eos
            def ids_to_str(ids):
                tokens = []
                for tid in ids:
                ...
                return " ".join(tokens)
            gt_str = ids_to_str(gt_ids)
            pr_str = ids_to_str(pr_ids)
            print(f"  [Sample {printed+1}] GT:   {gt_str}")
            print(f"              PRED: {pr_str}")
            printed += 1
...
def main() -> None:
    ...
    ap.add_argument("--num_workers", type=int, default=-1,
                    help="-1=auto (根据CPU核心数自动设置), 0=主进程加载")
    ...
    # 模型架构
    ap.add_argument("--decoder", choices=["lstm", "transformer"], default="lstm",
                    help="decoder类型: lstm (默认) 或 transformer")
    ap.add_argument("--encoder_dim", type=int, default=512)
    ap.add_argument("--decoder_dim", type=int, default=768)
    ap.add_argument("--embed_dim", type=int, default=384)
    ap.add_argument("--attention_dim", type=int, default=384)
    ap.add_argument("--num_heads", type=int, default=8)
    ap.add_argument("--decoder_layers", type=int, default=3)
    ap.add_argument("--dropout", type=float, default=0.3)
    ap.add_argument("--encoder_dropout", type=float, default=0.2)
    ...
    ap.add_argument("--use_focal", action="store_true", default=False)
    ...
    # 数据增强
    ap.add_argument("--augment_level", choices=["none", "light", "medium", "strong"], default="strong")
    
    # 其他
    ap.add_argument("--amp", action="store_true", help="使用混合精度训练")
    ap.add_argument("--resume", action="store_true")
    ap.add_argument("--eval_beam", action="store_true", help="use beam search")
    ap.add_argument("--beam_width", type=int, default=10)
    ap.add_argument("--checkpoint_dir", default="checkpoints_e2e_v3")
    ...
    if args.device == "cuda" and torch.cuda.is_available():
        ...
        print("⚠️  警告: 请求使用CUDA但GPU不可用，将使用CPU")
        print("   请检查CUDA和PyTorch是否正确安装")
    ...
    # 尝试从旧目录加载词汇表
    old_vocab_path = Path("checkpoints_e2e") / "vocab.json"
    if old_vocab_path.exists() and not vocab_path.exists():
        token2id, id2token = load_vocab(str(old_vocab_path))
    ...
    # 3) 数据集
    train_ds = E2EFormulaDatasetV3(train_pairs, token2id, cfg, augment=True)
    val_ds = E2EFormulaDatasetV3(val_pairs, token2id, cfg, augment=False)

    # Auto num_workers: 根据CPU核心数自动设置
    # 经验值：GPU训练时用 min(cpu_count, 4)，CPU训练时用 min(cpu_count-1, 2)
    # 过多的worker反而会因进程间通信开销降低效率
    if args.num_workers < 0:
        cpu_count = os.cpu_count() or 1
        if device.type == "cuda":
            args.num_workers = min(cpu_count, 4)
        else:
            args.num_workers = min(max(cpu_count - 1, 0), 2)
        print(f"Auto num_workers = {args.num_workers} (CPU cores={cpu_count})")

    # Auto batch size: 根据显存自动选择
    if args.batch_size <= 0:
        if device.type == "cuda":
            gpu_mem_gb = torch.cuda.get_device_properties(0).total_memory / 1024**3
            is_transformer = (args.decoder == "transformer")
            if gpu_mem_gb >= 16:
                args.batch_size = 16 if is_transformer else 24
            elif gpu_mem_gb >= 8:
                args.batch_size = 8 if is_transformer else 12
            elif gpu_mem_gb >= 6:
                args.batch_size = 4 if is_transformer else 8
            else:
                args.batch_size = 2 if is_transformer else 4
        else:
            args.batch_size = 4 if args.decoder == "transformer" else 8
        print(f"Auto batch_size = {args.batch_size} (decoder={args.decoder})")
    ...
    model_cfg = {
        ...
        # 维度默认与decoder_dim一致，避免LSTM(768) vs Transformer(512)的不对称
        "transformer_hidden_dim": args.decoder_dim,
        "transformer_nhead": args.num_heads,
        "transformer_num_layers": args.decoder_layers,
        "transformer_dropout": args.dropout,
    }
    model = build_e2e_model_v3(vocab_size, model_cfg).to(device)
    ...
    if args.resume and last_ckpt.exists():
        ...
        print(f"已恢复：epoch={start_epoch} best_seq_acc={best_val_seq_acc:.4f} best_tok_acc={best_val_tok_acc:.4f}")
    ...
    csv_path = log_dir / "train_e2e_v3_log.csv"
    ...
    with csv_path.open("a", newline="", encoding="utf-8") as f:
        ...
        if write_header:
            writer.writerow([
                ...
                "val_loss", "val_tok_acc", "val_seq_acc", "val_tok_acc_decode",
                "lr", "tf_ratio", "time"
            ])
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            val_seq_acc, val_tok_acc_decode = sequence_exact_match(
                model=model,
                loader=val_loader,
                device=device,
                bos_id=bos_id,
            ...
            print(
                ...
                f"seq_acc={val_seq_acc:.4f} tok_acc={val_tok_acc_decode:.4f} | "
                f"lr={lr:.2e} tf={tf_ratio:.2f} | {t1-t0:.1f}s"
            )

            # 每10轮打印前3条 GT vs PRED
            if (epoch + 1) % 10 == 0:
                print_gt_vs_pred(
                    model=model,
                    loader=val_loader,
            ...
            writer.writerow([
                ...
                f"{val_seq_acc:.6f}", f"{val_tok_acc_decode:.6f}",
            ...
            if val_seq_acc > best_val_seq_acc:
                ...
                best_val_tok_acc = val_tok_acc_decode
                state["best_val_seq_acc"] = best_val_seq_acc
                state["best_val_tok_acc"] = best_val_tok_acc
                save_checkpoint(best_ckpt, state)
                print(f"  ★ 新最佳！seq_acc={best_val_seq_acc:.4f} tok_acc={best_val_tok_acc:.4f}")

            # Scheduled Sampling: epoch-based teacher forcing schedule
            if epoch < 50:
                tf_ratio = 1.0
            elif epoch < 100:
                tf_ratio = 0.7
            ...
            if early_stopping(val_seq_acc):
                print(f"早停触发！epoch {epoch}，最佳seq_acc={best_val_seq_acc:.4f}")
                break

    print(f"\n训练完成！")
    print(f"最佳序列准确率: {best_val_seq_acc:.4f}")
    print(f"最佳Token准确率: {best_val_tok_acc:.4f}")
    print(f"best: {best_ckpt.resolve()}")
    print(f"\n提示: 使用 --resume 参数可以从断点继续训练")
